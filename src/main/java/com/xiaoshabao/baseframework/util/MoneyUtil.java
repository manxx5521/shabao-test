...
public class MoneyUtil {
  ...
  public static String CNValueOf(String moneyValue) {
    ...
    String curCN = null;
    if(len == 13 && cnt == 4 && "1000000000000".equals(int_value)){
      partValue = int_value.substring(0, partLen);
      int_value = int_value.substring(partLen);
      if(cnt == 4 && "000000000000".equals(int_value)){
        cn_currency.append("壹万亿");
      }
    } else{
    for (int i = 0; i < cnt; i++) {
      partValue = int_value.substring(0, partLen);
...
//    int fv3 = Integer.parseInt(fraction_value.substring(2, 3));
    if (fv1 + fv2 == 0) {
...
//      cn_currency.append(CN_Digits[fv3]).append("厘");
    }
  ...
  public static void main(String[] args) {
    System.out.println(CNValueOf("11451231321.101"));
  }
}

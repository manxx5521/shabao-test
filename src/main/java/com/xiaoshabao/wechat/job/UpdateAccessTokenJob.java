	...
	private void updateToken (TokenUpdateDto result){
		try {
			...
			if(result.getTokenState()==1){
				tokenService.updateAccessToken(result.getAccountId(), (AccessToken)result, time_now_long);
			}
			//更新jsToken
			if(result.getTokenState()==1){
				tokenService.updateJSToken(result.getAccountId(), (AccessToken)result, time_now_long);
			}
		} catch (Exception e) {
